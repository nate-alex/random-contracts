...
contract tinyboxes
{
	...
	function perpetualRender(uint256 seed, uint256 colorCount, uint256 shapeCount, int256 xPos, int256 yPos, int256 boxWidth, int256 boxWidthVariance, int256 boxHeight, int256 boxHeightVariance, int256 boxDensity) public view returns (string memory){


		bytes32[] memory pool = Random.initLatest(4, seed);
		
		colorValues = new uint256[](colorCount);
		...
		for(i=0; i<shapeCount; i++) {
			 ...
			 xpos = Random.uniform(pool, -(xPos), xPos) + (Random.uniform(pool, 0 , boxDensity-1) * 880/(boxDensity));
			 ypos = Random.uniform(pool, -(yPos), yPos) + (Random.uniform(pool, 0 , boxDensity-1) * 880/(boxDensity));
			 width = uint256(Random.uniform(pool, boxWidth - boxWidthVariance, boxWidth + boxWidthVariance));
			 height = uint256(Random.uniform(pool, boxHeight - boxHeightVariance, boxHeight + boxHeightVariance));
