...
contract MachiX is ERC721Full, Ownable {
    using SafeMath for uint256;
    ...
    mapping (uint256 => string) internal mediaTagStorage;
    mapping (uint256 => string) internal mediaCopyrightStorage;
    mapping (uint256 => string) internal creationDateStorage;
    ...
    event newArtAddtData(uint256 tokenID, string mediaTag, string mediaCopyright, string creationDate, string fileType);
    ...
    constructor() ERC721Full("Machi X", "MCXA") public {
        fileIPFSReferenceURL = "https://gateway.pinata.cloud/ipfs/";
        fileArweaveReferenceURL = "https://arweave.rocks/";
        tokenReferenceURI = "https://machiX.azurewebsites.net/api/HttpTrigger?id=";
    }
    ...
    function getAdditionalMetadata(uint256 tokenID) public view returns (string memory mediaTagByID, string memory mediaCopyrightByID, string memory creationDate, string memory fileTypeByID) {
      mediaTagByID = mediaTagStorage[tokenID];
      mediaCopyrightByID = mediaCopyrightStorage[tokenID];
      creationDate = creationDateStorage[tokenID];
      fileTypeByID = fileTypeStorage[tokenID];
    }
    
    function getRoyaltyData(uint256 tokenID) public view returns (address machiXAddress, uint256 royaltyFeeByID) {
      machiXAddress = 0x242c1972C683cC0713c885e29e2eBA0e1E854498;
    ...
    function updateIPFSLink(string memory newIPFSURL) public onlyOwner {
        fileIPFSReferenceURL = newIPFSURL;
    }
    
    function updateArweaveLink(string memory newArweaveURL) public onlyOwner {
    ...
    function createForArtist(string memory fileIPFSHash, string memory fileArweaveHash, string memory artistName, string memory artTitle, string memory artistNote, string memory mediaTag, string memory mediaCopyright, uint256 royaltyFee, uint256 totalCap, string memory fileType, string memory creationDate) public onlyOwner {
    ...
    mediaTagStorage[totalArtPieces] = mediaTag;
    mediaCopyrightStorage[totalArtPieces] = mediaCopyright;
    ...
    creationDateStorage[totalArtPieces] = creationDate;
    fileTypeStorage[totalArtPieces] = fileType;
    
    _registerToken();
    
    emit NewArtMetadata(totalArtPieces, artistName, artTitle, artistNote, i, totalCap);
    emit newArtAddtData(totalArtPieces, mediaTag, mediaCopyright, creationDate, fileType);
    emit NewArtImageRefs(totalArtPieces, fileIPFSHash, fileArweaveHash);
    emit NewArtRoyaltyInfo(totalArtPieces, royaltyFee, 0x242c1972C683cC0713c885e29e2eBA0e1E854498);
    ...
    function _registerToken() private {
        ...
        _mint(0x242c1972C683cC0713c885e29e2eBA0e1E854498, tokenId);
